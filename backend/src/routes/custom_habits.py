...
def update_custom_habits(custom_habit_id: str):
    ...
    try:
        custom_habit: CustomHabit | None = CustomHabit.update(
            custom_habit_id, data)
    except ValueError as e:
        abort(400, str(e))
...
def get_user_custom_habits():
    ...
    """
    current_user_id = get_jwt_identity()
    custom_habits: list[CustomHabit] = CustomHabit.query.filter_by(
        habit_owner_id=current_user_id).all()
